package com.jazzysoft.jpctsample;
...
import com.jazzysoft.jpctsample.joystick.JoystickMovedListener;
import com.jazzysoft.jpctsample.joystick.JoystickView;
import com.threed.jpct.SimpleVector;
import com.threed.jpct.Texture;
import com.threed.jpct.TextureManager;
import com.threed.jpct.util.BitmapHelper;

public class ViewerActivity extends Activity implements OnTouchListener, RenderView.CollisionListener, JoystickMovedListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		this.findViewById(R.id.buttonLoadObj2).setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				myRenderer.addObject(ViewerActivity.this, RenderView.OBJ_TYPES.CAMARO);
			}
		});
		...
		this.findViewById(R.id.buttonLoadObj3).setOnClickListener(new View.OnClickListener() {
			@Override
			public void onClick(View v) {
				myRenderer.addObject(ViewerActivity.this, RenderView.OBJ_TYPES.POLICE_CAR);
			}
		});
		...
		this.findViewById(R.id.buttonLoadObj4).setOnClickListener(new View.OnClickListener() {
			@Override
			public void onClick(View v) {
				myRenderer.addObject(ViewerActivity.this, RenderView.OBJ_TYPES.TRUCK);
			}
		});
		...
		this.findViewById(R.id.ButtonAxisZPlus).setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				if (currentTransformation == TRANSFORMATION.ROTATION) {
					myRenderer.applyRotation(RenderView.AXIS.Z, 0.5f);
				} else {
	...
	public boolean onTouch(View v, MotionEvent event) {
		if (event.getAction() == MotionEvent.ACTION_DOWN) {
			...
			if (currentTransformation == TRANSFORMATION.ROTATION) {
				if (diffX > 0) {
					myRenderer.applyRotation(RenderView.AXIS.X, 0.1f);
				} else if (diffX < 0) {
					myRenderer.applyRotation(RenderView.AXIS.X, -0.1f);
				}

				if (diffY > 0) {
					myRenderer.applyRotation(RenderView.AXIS.Y, 0.1f);
				} else if (diffY < 0) {
					myRenderer.applyRotation(RenderView.AXIS.Y, -0.1f);
				}
